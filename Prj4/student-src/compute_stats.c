...
void compute_stats(stats_t *stats)
{
	...
	uint64_t tlbtime = (stats->accesses)*(stats->TLB_READ_TIME+stats->MEMORY_READ_TIME);
	uint64_t memorytime = stats->MEMORY_READ_TIME*stats->translation_faults;
	uint64_t disktime = (stats->reads_from_disk)*(stats->DISK_READ_TIME)+(stats->writes_to_disk)*(stats->DISK_WRITE_TIME);
	stats->AAT = (tlbtime+ memorytime + disktime)/((double)(stats->accesses));
}
